...
const useSlidingElements = (
  slidingElements: SlidingElementConfig[],
  disabled: boolean
) => {
  ...
  useEffect(() => {
    ...
    setInterval(() => {
      slidingElements.forEach((el, i) => {
        ...
        const offsetY =
          perlinY +
          (height / 2 - (elRect.x + elRect.height / 2) + el.initialOffset.y) *
            el.velocity.y;
...
const floatEffect = (seed: number, noiseStep: number) => {
  ...
  const perlinX = (noise2d(noiseStep, 0) - 0.5) * 5;
  const perlinY = (noise2d(0, noiseStep) - 0.5) * 5;
